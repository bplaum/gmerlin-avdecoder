...
static int open_rawaudio(bgav_demuxer_context_t * ctx)
  ...
  if(!strncasecmp(mimetype, "audio/L16", 9))
    ...
    s->ci->block_align = 2 * s->data.audio.format->num_channels;
  ...
  if((ctx->input->total_bytes > 0))
    {
    s->stats.pts_start = 0;
    s->stats.pts_end     = ctx->input->total_bytes / s->ci->block_align;
...
static gavl_source_status_t next_packet_rawaudio(bgav_demuxer_context_t * ctx)
  ...
  bytes_to_read = SAMPLES_PER_FRAME * s->ci->block_align;
  ...
  p->pts = ctx->input->position / s->ci->block_align;
...
static void seek_rawaudio(bgav_demuxer_context_t * ctx, int64_t time, int scale)
  ...
  STREAM_SET_SYNC(s, (int64_t)file_position / s->ci->block_align);
